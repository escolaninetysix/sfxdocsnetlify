...
Blobs Wipe transitions between two layers by animating an evolving, random blobby pattern of transparency which progressively reveals the underlying layer as the size of the transparent regions increases.


[![BCC_Blob_Wipe](https://borisfx-com-res.cloudinary.com/image/upload//documentation/continuum/uploads/2014/04/BCC_Blob_Wipe.jpg)](https://borisfx-com-res.cloudinary.com/image/upload//documentation/continuum/uploads/2014/04/BCC_Blob_Wipe.jpg)
...
**Softness** softens the edges of the blob shapes.


**Scale** modifies the overall size of the blob pattern.  Increasing Scale creates fewer and larger blobs.


**Stretch Width** stretches the blob pattern horizontally.


**Offset**shifts the position of the overall blob pattern.


**Turbulence**controls the complexity of the blob pattern.  Higher values create increasingly more intricate and detailed blob patterns whereas lower values create simpler and smoother blob shapes.


**Turbulence Feedback**modifies the shape of the blob pattern with higher values producing more gnarled shapes or ringing patterns.


**Turbulence Sub Scaling**controls the size of the additional blob complexity that appears with increasing turbulence.  Larger values tend to produce larger, smoother blobs whereas smaller values tend to addition additional fine detail to the blob shapes.


**Seed** controls the random starting point for the blob animation.  Adjust the seed parameter to creating blob patterns with the same overall look and feel but with differently randomized blob positions and shapes.


 


**Blob Animation Group**


**Flow Rate** set the speed of the motion of the Fractal Noise texture pattern.


**Flow Direction** sets the direction of the motion of the Fractal Noise texture pattern.


**Morph Rate** determines the rate at which the Fractal Noise texture surges and rotates inward as the filter auto-animates.
