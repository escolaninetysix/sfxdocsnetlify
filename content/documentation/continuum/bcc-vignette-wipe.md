...
Vignette Wipe transitions between two layers by animating a softened elliptical/rectangular opacity vignette which progressively reveals the underlying layer as the size of the vignette changes.


[![BCC_Vignette_Wipe](https://borisfx-com-res.cloudinary.com/image/upload//documentation/continuum/uploads/2013/10/BCC_Vignette_Wipe.jpg)](https://borisfx-com-res.cloudinary.com/image/upload//documentation/continuum/uploads/2013/10/BCC_Vignette_Wipe.jpg)
...
The **Animation menu** controls whether the filter auto-animates, uses a Percent Done slider (which much be keyframed), or offers direct manual controls over the vignette radius. Choose from the following options.


* *Manual* is the most flexible setting. Use Manual if you are applying the filter as a composite instead of a transition, or do not have enough control with the other settings. When Manual is chosen, you create the wipe by manually animating parameters such as Radius.  When Manual is chosen, Percent Done has no affect.
...
If an option other than Manual is chosen, the filter automatically adjusts the overall pace of the animation to ensure the transition completes on time for any set of effect parameters. If this does not produce the desired look at the ends of the effect, set Animation to Manual and manually adjust the effect instead.


 


**Animation Tuning Group:** Contains contextually enabled parameters to control the transition animation.
...
**Center** controls set the center point of the wipe geometry.


**Radius** allows manual control over the radius of the vignette size.  This parameter is only available when the Animation menu is set to Manual.


**Squareness** controls whether the vignette shape is elliptical, rectangular, or somewhere in between.


**Scale Others to Radius** when enabled, this automatically re-scales the geometric properties such as softness as the radius animates over the course of the transition.


**Softness** softens the edges of the wipe by increasing transparency in pixels that are close to the edges.


**Smooth Softness** this controls the shape of the transition between the masked and unmasked parts of the image. A value of zero produces a sharper change between regions, whereas higher values produce smoother, more natural gradations.
...
The **Texture Type**controls which type(s) of texture map influences the wipe.
