{
   "title": "Mocha 6.1.0 Python Scripting Guide"
}
:skip-front-matter:
:Author:    (C) Boris FX
:Email:     support@borisfx.com
:Date:      05/06/2019
:Revision:  Mocha 2019.5
:doctype: article
:toc:
...
== Deprecated API in Mocha Pro 2019

These items are now deprecated as of *Mocha Pro 2019* onwards:

We have moved to Qt5 for *Mocha Pro 2019* and therefore have also upgraded to the supported Python integrations for that release.
...
ParameterSet has the following read-only properties:

* `keyframes` accumulates all child keyframes recursively
* `parameters` accumulates child *parameters* on the 1st level (i.e. not recursively)
* `subsets` accumulates child *parameter sets* on the 1st level (i.e. not recursively)
