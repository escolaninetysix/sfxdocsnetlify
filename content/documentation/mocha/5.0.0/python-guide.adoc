{
   "title": "Mocha 5.0.0 Python Scripting Guide"
}
:skip-front-matter:
:Author:    (C) Imagineer Systems Ltd.
:Email:     support@imagineersystems.com
:Date:      01/06/2016
:Revision:  Mocha Pro v5.0.0
:doctype: article
:icons:
...
.The mocha Python command tools are available per system in the following default locations:
...
== Running mocha Python

To run a mocha script with the mocha Python version, call it directly with the script, like so:

* *OS X:* '/Applications/mocha Pro.app/Contents/MacOS/python myMochaScript.py'
* *Windows:* 'C:\Program Files\mocha Pro V5\python\python.exe myMochaScript.py'
* *Linux:* '/opt/isl/mochaV5/python/bin/python myMochaScript.py'
...
==== Exporting a rendered Remove, Insert or Stabilization
...
* *layers (list of Layer instances)*: Layers which will be exported.
* *clip (Clip)*: Clip instance for those layers.
...
* *suffix (str)*: Any suffix you want at the end of the file name
* *index_start (int)*: The start frame to export
* *index_finish (int)*: The end frame to export
* *index_width (int)*: Digits count in clip index.
...
class NukeRotoPaintExtra(AbstractShapeDataExporter):
    def __init__(self):
        super(NukeRotoPaintV2, self).__init__("Nuke RotoPaint [Basic] Extra (*.nk)", "", number_of_data_streams=1,
...
Stop frame index. The out point for your render.
...
The following command performs removing contents of Layer 2 and saves entire result clip to the /tmp/rendered directory.

.Command
----
$ python mocharender.py --project Markers.mocha "Layer 2"  --output-directory="/tmp/rendered" --output-extension=png --render-type=remove -v4
----
