...
You can now access frame image data on any frame for any clip in mocha to process or manipulate pixels.
This will return an `ImageData` instance with the pixel data and metadata of the querying clip source image.

See examples under  <<image_access, Image Access and Creating New Clips>> for more details.

=== Image output to clip

You can also now write images to clips by creating `Project.new_output_clip` and then assigning the clip new `ImageData`.

See examples under <<image_access, Image Access and Creating New Clips>> for more details.

=== Playhead access
You can now access the playhead to get and set the current time.

See  <<playhead_access, Getting and Setting the Project Timeline Playhead>> for more details.

== Depreciated API in {revision}

These items are now deprecated as of version 5.5.2:
...
Adding an icon using the +setIcon+ function command from 'PySide.QtGui' will still define the icon on the the toolbar if your icon file path does not exist.
...
class SpotCleaner(AbstractTool):
    def __init__(self, project):
        ...
        action.setIcon(QIcon('/myicons/spotclean.png'))
        AbstractTool.__init__(self, action)
        action.setParent(self)
        tools_bar = ui.find_widget('ToolsBar', QToolBar)
        tools_bar.addAction(action)


    def create_spot(self, pos):

        new_layer = self.proj.add_layer(self.proj.clips[0], name='spot', frame_number=0, view=0)
...
Path to the output directory for the rendered export. Note that `--output-directory` is now depreciated as it wasn't clear this was the export directory.
...
Start frame index. The in point for your render. However, this is depreciated and it is better to use --frames (see below).

*-O FRAME_OUT, --frame-out=FRAME_OUT* +
Stop frame index. The out point for your render. However, this is depreciated and it is better to use --frames (see below).
